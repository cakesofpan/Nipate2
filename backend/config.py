...
# Always load the project-root .env, regardless of the directory the backend
# is launched from (so `python backend/main.py` works the same as `python main.py`).
_PROJECT_ROOT = Path(__file__).resolve().parents[1]
load_dotenv(dotenv_path=_PROJECT_ROOT / ".env")
load_dotenv()  # also pick up any cwd-level .env / shell overrides
...
# Required for projects that still use a Legacy JWT Secret (Settings → API →
# JWT Settings). The auth middleware falls back to Supabase's JWKS endpoint when
# this is absent, but for legacy-secret projects the secret MUST be present or
# every token will be rejected.
SUPABASE_JWT_SECRET: str = os.getenv("SUPABASE_JWT_SECRET", "")
if not SUPABASE_JWT_SECRET:
    raise RuntimeError(
        "SUPABASE_JWT_SECRET is not set. Loaded .env from "
        f"{_PROJECT_ROOT / '.env'}. For legacy-secret Supabase projects this "
        "secret is required to verify tokens — copy it from Supabase → "
