...
def _get_jwks_client() -> PyJWKClient:
    ...
    Supabase's JWKS endpoint requires the anon `apikey` header (or it returns
    401 "No API key found in request"), so we pass SUPABASE_ANON_KEY as a
    header on every JWKS request.
    """
    global _jwks_client
...
def verify_jwt(token: str) -> dict:
    ...
    if last_exc is not None:
        raise last_exc
    log.error(
        "JWT verification failed for token alg=%s. This almost always means "
        "SUPABASE_JWT_SECRET in .env does not match how the Supabase project signs "
        "tokens (e.g. the project uses asymmetric JWT Signing Keys / ES256 while "
        ".env holds the legacy HS256 secret, or JWKS is unreachable at "
        "%s/auth/v1/jwks). Fix in Supabase Dashboard → Settings → API → JWT Settings, "
        "or set SUPABASE_JWT_SECRET to the matching secret / enable JWKS.",
        unverified_alg, SUPABASE_URL,
    )
    raise pyjwt.InvalidTokenError(
